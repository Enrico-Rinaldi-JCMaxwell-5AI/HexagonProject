...
public class BallScript : MonoBehaviour {
    ...
    void Start () {
        gm = GameObject.Find("SplitScreenGameManager");
    }
	...
	void Update () {
        if (ground)
        {
            ...
            if (velocity < (SplitScreenModel.getMultiplier(gm.GetComponent<GameController>().currentTime())) + SplitScreenModel.ballMinSpeed)
            {
            ...
            if (velocity > (SplitScreenModel.getMultiplier(gm.GetComponent<GameController>().currentTime())) + SplitScreenModel.ballMaxSpeed)
            {
    ...
    private void FixedUpdate()
    {
        if (transform.position.y < 0.30f && !ground)
        {
            ...
            ground = true;
            gm.GetComponent<GameController>().centralCannonStop = false;
